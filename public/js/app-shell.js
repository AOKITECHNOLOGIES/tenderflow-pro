...
async function getDepartments() {
  ...
  if (!companyId) return [];
  const { data } = await supabase.from('departments').select('name').eq('company_id', companyId).order('name');
  if (data?.length) return data.map(d => d.name);
  const { data: profileDepts } = await supabase.from('profiles').select('department').not('department', 'is', null).eq('company_id', companyId);
    ...
    const html = `<div class="view-enter max-w-3xl space-y-6">
      ...
      <div class="bg-surface-800/40 border border-slate-700/40 rounded-xl overflow-hidden">
        ...
        <div class="p-5">
          ...
          : `<div class="tf-section-content px-4 py-3 bg-surface-900/60 border border-slate-600/50 rounded-lg text-slate-300 text-sm leading-relaxed min-h-48 max-w-none">${draft?.content || task.content || '<span class="text-slate-500">No content yet.</span>'}</div>`}
  ...
  'rfp-processor': async function() {
    ...
    window._doRFP = async () => {
      ...
      try {
        ...
        extractedText = extractedText.replace(/[^\x20-\x7E\u00A0-\u024F\n\r\t]/g, ' ').replace(/[ \t]{3,}/g, '  ').replace(/\n{4,}/g, '\n\n').trim();
        if (extractedText.length < 50) throw new Error('Could not extract readable text from this document. The PDF may use custom fonts or be image-based. Try a Word (.docx) version instead.');
...
${kbContext}`,
            messages: [{ role: 'user', content: `Process this RFP document and extract questions with answers:\n\n${extractedText.substring(0, 12000)}` }],
            max_tokens: 2000,
          },
        });
        let questions = [];
        ...
        if (questions.length === 0) {
          ...
          questions = source.map(q => ({ question: q, answer: kb.length ? 'Based on our company documentation, we confirm our capability and experience to fulfil this requirement. Our team meets all specified criteria.' : '[Upload Knowledge Base documents for AI-generated answers.]' }));
        }
        ...
        document.getElementById('rfp-content').innerHTML = window._rfpAns.map((x, i) =>
          `<div class="border border-slate-700 rounded-lg p-4"><p class="text-brand-400 text-sm font-medium mb-1">Q${i+1}: ${x.q.substring(0, 200)}</p><p class="text-slate-200 text-sm leading-relaxed">${x.a}</p></div>`
        ).join('');
      } catch (err) {
        document.getElementById('rfp-result').classList.remove('hidden');
        document.getElementById('rfp-content').innerHTML = `<div class="border border-red-500/20 bg-red-500/10 rounded-lg p-4"><p class="text-red-400 text-sm font-medium">Processing failed</p><p class="text-slate-300 text-sm mt-1">${err.message}</p></div>`;
      }
...
export function mountAppShell() {
  ...
  // Inject content formatting CSS into <head> once — scoped so it never leaks
  if (!document.getElementById('tf-content-styles')) {
    const s = document.createElement('style');
    s.id = 'tf-content-styles';
    s.textContent = `
      .tf-section-content { font-size:13px; line-height:1.8; color:#334155; }
      ...
      .tf-section-content h4,.tf-section-content h5,.tf-section-content h6 { font-size:13px;font-weight:600;margin:8px 0 3px; }
      .tf-section-content p  { margin:0 0 8px;line-height:1.75; }
      .tf-section-content ul,.tf-section-content ol { padding-left:1.5em;margin:3px 0 8px; }
      .tf-section-content li { margin-bottom:4px;line-height:1.7; }
      .tf-section-content strong,.tf-section-content b { font-weight:700; }
      .tf-section-content em,.tf-section-content i { font-style:italic; }
      .tf-section-content u  { text-decoration:underline; }
      .tf-section-content s,.tf-section-content strike { text-decoration:line-through; }
      ...
      .tf-section-content a { color:#0284c7; }
...
window._unlockTender = async (tenderId) => {
  ...
  const { error } = await supabase.from('tenders').update({ status: 'in_progress' }).eq('id', tenderId);
  if (error) { window.TF?.toast?.(`Unlock failed: ${error.message}`, 'error'); return; }
  await supabase.from('documents').update({ is_locked: false }).eq('tender_id', tenderId);
...
window._editUser = async (userId) => {
  ...
  const roleOptions = ['dept_user', 'bid_manager', 'it_admin', ...(isSuperAdmin() ? ['super_admin'] : [])].map(r => `<option value="${r}" ${u.role === r ? 'selected' : ''}>${r.replace(/_/g, ' ')}</option>`).join('');
  const modal = document.createElement('div');
  modal.className = 'fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm';
  modal.innerHTML = `<div class="bg-surface-800 border border-slate-700/50 rounded-2xl p-6 w-full max-w-md shadow-2xl">
    ...
    <div class="space-y-4">
      <div><label class="block text-sm text-slate-300 mb-1">Full Name</label><input id="eu-name" type="text" value="${u.full_name}" class="w-full px-3 py-2 bg-surface-900/60 border border-slate-600/50 rounded-lg text-white text-sm" /></div>
      <div><label class="block text-sm text-slate-300 mb-1">Role</label><select id="eu-role" class="w-full px-3 py-2 bg-surface-900/60 border border-slate-600/50 rounded-lg text-white text-sm">${roleOptions}</select></div>
      <div><label class="block text-sm text-slate-300 mb-1">Department</label><select id="eu-dept" class="w-full px-3 py-2 bg-surface-900/60 border border-slate-600/50 rounded-lg text-white text-sm"><option value="">— None —</option>${deptOptions}</select></div>
    </div>
    <div class="flex gap-3 mt-6">
  ...
  modal.querySelector('#eu-submit').addEventListener('click', async () => {
    const { error } = await supabase.from('profiles').update({ full_name: modal.querySelector('#eu-name').value.trim(), role: modal.querySelector('#eu-role').value, department: modal.querySelector('#eu-dept').value || null }).eq('id', userId);
...
window._manageDepartments = async () => {
  ...
  function renderDeptList(depts) {
    ...
    if (!list) return;
    if (!depts.length) { list.innerHTML = '<p class="text-sm text-slate-500 px-3">No departments yet.</p>'; return; }
    list.innerHTML = depts.map(d => `<div class="flex items-center justify-between px-3 py-2 bg-surface-900/60 rounded-lg"><span class="text-sm text-white">${d.name}</span><div class="flex gap-2"><button onclick="window._renameDepartmentById('${d.id}', '${d.name.replace(/'/g, "\\'")}')" class="text-xs text-brand-400 hover:text-brand-300">Rename</button><button onclick="window._deleteDepartment('${d.id}', '${d.name.replace(/'/g, "\\'")}')" class="text-xs text-red-400 hover:text-red-300">Delete</button></div></div>`).join('');
  }
  const modal = document.createElement('div');
  modal.className = 'fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm';
  modal.innerHTML = `<div class="bg-surface-800 border border-slate-700/50 rounded-2xl p-6 w-full max-w-md shadow-2xl">
    ...
    <div id="dept-list" class="space-y-2 mb-4 max-h-64 overflow-y-auto"><p class="text-xs text-slate-500 px-3">Loading...</p></div>
  ...
  modal.querySelector('#add-dept-btn').addEventListener('click', async () => {
    ...
    if (error) { errEl.textContent = error.code === '23505' ? `"${name}" already exists.` : error.message; errEl.classList.remove('hidden'); return; }
    input.value = '';
    window.TF?.toast?.(`Department "${name}" saved`, 'success');
    renderDeptList(await loadDepts());
  });
  modal.querySelector('#new-dept-input').addEventListener('keydown', (e) => { if (e.key === 'Enter') modal.querySelector('#add-dept-btn').click(); });
};
...
window._renameDepartmentById = async (deptId, oldName) => {
  ...
  const profile = getProfile();
  const companyId = _selectedCompanyId || profile.company_id;
  const { error } = await supabase.from('departments').update({ name: newName.trim() }).eq('id', deptId);
  if (error) { window.TF?.toast?.(`Rename failed: ${error.message}`, 'error'); return; }
  await supabase.from('profiles').update({ department: newName.trim() }).eq('department', oldName).eq('company_id', companyId);
...
window._deleteDepartment = async (deptId, name) => {
  ...
  const profile = getProfile();
  const companyId = _selectedCompanyId || profile.company_id;
  await supabase.from('profiles').update({ department: null }).eq('department', name).eq('company_id', companyId);
...
window._createUser = async () => {
  ...
  if (isSuperAdmin()) {
    ...
    companySelect = `<div><label class="block text-sm text-slate-300 mb-1">Company *</label><select id="nu-company" class="w-full px-3 py-2 bg-surface-900/60 border border-slate-600/50 rounded-lg text-white text-sm"><option value="">— Select —</option>${(companies || []).map(c => `<option value="${c.id}">${c.name}</option>`).join('')}</select></div>`;
  }
  const modal = document.createElement('div');
  modal.className = 'fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm';
  modal.innerHTML = `<div class="bg-surface-800 border border-slate-700/50 rounded-2xl p-6 w-full max-w-md shadow-2xl">
    ...
    <div class="space-y-4">
      <div><label class="block text-sm text-slate-300 mb-1">Full Name *</label><input id="nu-name" type="text" placeholder="Jane Doe" class="w-full px-3 py-2 bg-surface-900/60 border border-slate-600/50 rounded-lg text-white text-sm" /></div>
      <div><label class="block text-sm text-slate-300 mb-1">Email *</label><input id="nu-email" type="email" placeholder="jane@company.com" class="w-full px-3 py-2 bg-surface-900/60 border border-slate-600/50 rounded-lg text-white text-sm" /></div>
      <div><label class="block text-sm text-slate-300 mb-1">Temporary Password *</label><input id="nu-password" type="text" placeholder="Min 8 characters" class="w-full px-3 py-2 bg-surface-900/60 border border-slate-600/50 rounded-lg text-white text-sm font-mono" /></div>
      <div class="grid grid-cols-2 gap-3">
        <div><label class="block text-sm text-slate-300 mb-1">Role</label><select id="nu-role" class="w-full px-3 py-2 bg-surface-900/60 border border-slate-600/50 rounded-lg text-white text-sm">${roleOptions}</select></div>
        <div><label class="block text-sm text-slate-300 mb-1">Department</label><select id="nu-dept" class="w-full px-3 py-2 bg-surface-900/60 border border-slate-600/50 rounded-lg text-white text-sm"><option value="">— None —</option>${deptOptions}</select></div>
      </div>
      ${companySelect}
    </div>
    <div class="flex gap-3 mt-6">
  ...
  modal.querySelector('#nu-submit').addEventListener('click', async () => {
    ...
    const { data: { session } } = await supabase.auth.getSession();
    const { data, error } = await supabase.functions.invoke('create-user', { body: { email, password, full_name: name, role, department: dept || null, company_id: companyId || null }, headers: { Authorization: `Bearer ${session.access_token}` } });
...
window._loadTaskAssignFields = async (taskId) => {
  ...
  const { data: users } = await supabase.from('profiles').select('id, full_name, department').eq('company_id', companyId).eq('is_active', true).order('full_name');
  const { data: task, error: taskErr } = await supabase.from('tasks').select('assigned_to, due_date, priority, is_mandatory').eq('id', taskId).maybeSingle();
  if (taskErr) { console.error('[Assign] Task fetch error:', taskErr.message); return; }
  const userOptions = (users || []).map(u => `<option value="${u.id}" ${task?.assigned_to === u.id ? 'selected' : ''}>${u.full_name}${u.department ? ` (${u.department})` : ''}</option>`).join('');
  container.innerHTML = `
    ...
    <div class="grid grid-cols-2 gap-3">
      ...
      <div><label class="block text-xs text-slate-400 mb-1">Due Date</label>
        <input id="ta-due" type="datetime-local" value="${task?.due_date ? task.due_date.slice(0,16) : ''}" class="w-full px-3 py-2 bg-surface-900/60 border border-slate-600/50 rounded-lg text-white text-sm" /></div>
    </div>
    <label class="flex items-center gap-2 text-sm text-slate-300">
...
window._saveTaskAssignment = async (taskId) => {
  ...
  const profile = getProfile();
  const { error } = await supabase.from('tasks').update({ assigned_to: assignTo || null, assigned_by: assignTo ? profile.id : null, priority, due_date: dueDate || null, is_mandatory: isMandatory, status: assignTo ? 'assigned' : 'unassigned' }).eq('id', taskId);
  if (error) { const errEl = document.getElementById('task-assign-error'); if (errEl) { errEl.textContent = error.message; errEl.classList.remove('hidden'); } return; }
  ...
  modal.innerHTML = `<div class="bg-surface-800 border border-slate-700/50 rounded-2xl p-6 w-full max-w-md shadow-2xl">
    ...
    <div class="space-y-4">
      <div>
        ...
        <div onclick="document.getElementById('id-file').click()" class="border-2 border-dashed border-slate-600 rounded-xl p-5 text-center cursor-pointer hover:border-brand-500 transition-colors">
      ...
      <div>
        ...
        <div onclick="document.getElementById('id-tmpl-file').click()" class="border-2 border-dashed border-slate-600 rounded-xl p-5 text-center cursor-pointer hover:border-brand-500 transition-colors">
          <p class="text-2xl mb-1">📝</p>
...
window._deleteUserPrompt = async (userId, userName) => {
  ...
  modal.querySelector('#du-hard').addEventListener('click', async () => {
    ...
    const { data: { session } } = await supabase.auth.getSession();
    await supabase.functions.invoke('create-user', { body: { _action: 'delete', user_id: userId }, headers: { Authorization: `Bearer ${session.access_token}` } });
...
window._toggleAllTenders = (checked) => { document.querySelectorAll('.tender-checkbox').forEach(cb => cb.checked = checked); window._updateTenderBulkBar(); };
...
window._clearTenderSelection = () => { document.querySelectorAll('.tender-checkbox').forEach(cb => cb.checked = false); const selectAll = document.getElementById('tender-select-all'); if (selectAll) selectAll.checked = false; window._updateTenderBulkBar(); };
window._bulkDeleteTenders = async () => {
  ...
  for (const id of ids) { await supabase.from('tasks').delete().eq('tender_id', id); await supabase.from('documents').delete().eq('tender_id', id); await supabase.from('tenders').delete().eq('id', id); }
  window.TF?.toast?.(`${ids.length} tender(s) deleted`, 'success');
  const route = getCurrentRoute(); if (route) refreshView(route);
};

window._toggleAllTasks = (checked) => { document.querySelectorAll('.task-checkbox').forEach(cb => cb.checked = checked); window._updateTaskBulkBar(); };
...
window._clearTaskSelection = () => { document.querySelectorAll('.task-checkbox').forEach(cb => cb.checked = false); const selectAll = document.getElementById('task-select-all'); if (selectAll) selectAll.checked = false; window._updateTaskBulkBar(); };
...
window._insertTable = () => {
  ...
  modal.innerHTML = `<div class="bg-surface-800 border border-slate-700/50 rounded-2xl p-6 w-full max-w-xs shadow-2xl">
    ...
    <div class="space-y-3">
      <div class="flex items-center gap-3"><label class="text-xs text-slate-400 w-16">Rows</label><input id="tbl-rows" type="number" value="3" min="1" max="20" class="flex-1 px-3 py-1.5 bg-surface-900/60 border border-slate-600/50 rounded-lg text-white text-sm" /></div>
      <div class="flex items-center gap-3"><label class="text-xs text-slate-400 w-16">Columns</label><input id="tbl-cols" type="number" value="3" min="1" max="10" class="flex-1 px-3 py-1.5 bg-surface-900/60 border border-slate-600/50 rounded-lg text-white text-sm" /></div>
    </div>
  ...
  modal.querySelector('#tbl-insert').addEventListener('click', () => {
    ...
    if (range) {
      ...
      if (leaf && leaf.domNode) { let node = leaf.domNode; while (node && node.parentNode !== editorRoot) node = node.parentNode; insertAfter = node; }
    }
    if (insertAfter && insertAfter !== editorRoot) { editorRoot.insertBefore(table, insertAfter.nextSibling); } else { editorRoot.appendChild(table); }
    const p = document.createElement('p'); p.innerHTML = '<br>'; table.after(p);
    table.querySelectorAll('td, th').forEach(cell => { cell.setAttribute('contenteditable', 'true'); cell.style.cursor = 'text'; });
    editor.update();
  });
};
...
window._tableAddRow = () => {
  ...
  for (let i = 0; i < cols; i++) { const td = document.createElement('td'); td.style.cssText = 'border:1px solid #475569;padding:6px 10px;min-width:80px;background:#0f172a;color:#e2e8f0;'; td.setAttribute('contenteditable', 'true'); newRow.appendChild(td); }
...
window._tableAddCol = () => {
  ...
  table.querySelectorAll('tr').forEach((row, ri) => {
    ...
    const ref = row.children[colIdx + 1] || null; row.insertBefore(newCell, ref);
  });
};
